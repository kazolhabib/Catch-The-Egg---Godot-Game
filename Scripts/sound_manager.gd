...
func create_headless_audio():
	for item in get_sfx_plan():
		sfx[item.name] = create_stream(item.name, item.duration)
...
func get_sfx_plan() -> Array[Dictionary]:
	return [
		...
		{"name": "egg_break", "duration": 0.30},
		{"name": "poop_splat", "duration": 0.34},
...
func create_loading_overlay():
	...
	backdrop.color = Color(0.05, 0.08, 0.10, 0.82)
	...
	loading_bar.set_anchors_preset(Control.PRESET_BOTTOM_WIDE)
	loading_bar.offset_left = 18
	loading_bar.offset_right = -18
	loading_bar.offset_top = -34
	loading_bar.offset_bottom = -12
...
func play_sfx(sound_name: String, volume_db: float = 0.0, pitch_scale: float = 1.0):
	...
	if get_sfx_player_count() >= MAX_PLAYERS:
...
func create_music_stream() -> AudioStreamWAV:
	...
	return create_wav_stream(MUSIC_DURATION, func(time: float, _index: int) -> float:
		return get_music_sample(time) * 0.62
	, true, frame_count)
...
func get_sample(sound_name: String, time: float, duration: float, index: int) -> float:
	match sound_name:
		...
		"egg_break":
			var sharp_crack := crack_noise(index, time, 0.050, 0.62)
			var shell_bits := crack_noise(index + 117, time - 0.045, 0.140, 0.30)
			var floor_knock := low_thump(142.0, 62.0, time, duration, 0.16)
			var tiny_scatter := hit_noise(index + 251, time - 0.115, 0.11, 0.12)
			return sharp_crack + shell_bits + floor_knock + tiny_scatter
		"poop_splat":
			var slap := wet_noise(index, time, 0.16, 0.52)
			var wet_tail := low_thump(78.0, 36.0, time, duration, 0.32)
			var smear := wet_noise(index + 211, time - 0.090, 0.23, 0.28)
			var sticky_pop := wet_noise(index + 301, time - 0.190, 0.095, 0.12)
			return slap + wet_tail + smear + sticky_pop
		"jump":
